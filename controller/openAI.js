...
// Google Translate API 인증
const auth_translate = new google.auth.JWT({
  email: serviceAccount.client_email,
  key: serviceAccount.private_key,
...
const translate = google.translate({ version: "v2", auth: auth_translate });
...
const openAIController = {
  ...
  postOpenAIPsychologicalAnalysis: async (req, res) => {
    ...
    let parseData, parseMessageArr, parsingScore, parsingType, parsepUid;
    // yourMailAddr = "",
    // myMailAddr = "",
    // myMailPwd = "";
    ...
    try {
      ...
      const { messageArr, type, score, pUid, en } = parseData;
      console.log(
      ...
      // const transAnalyzeMsg = await translateText(analyzeMsg);
...
const ellaFamilyController = {
  ...
  postOpenAIEllaFamilyTraning: async (req, res) => {
    ...
    let promptArr = []; // 삽입 Prompt Array
    let temperature = 1;
    const codeArr = ["room", "homework", "phone"];
    try {
      if (typeof data === "string") {
        parseData = JSON.parse(data);
      } else parseData = data;
      ...
      switch (code) {
        case "room":
          promptArr.push({
            role: "system",
            content: `다음 기준에 따라 user와 역할극을 하라.
1) assistant 역할: 잔소리하는 부모, user 역할: 자녀
2) assistant는 자녀가 방 정리를 제대로 하지 않는다고 생각하고 잔소리를 한다. 방정리를 꼭 해야 하는 이유를 설명하거나, 잘못을 지적하거나, 과거 행동을 이야기할 수 있다. 한 번에 최대 70자까지 말할 수 있고, 반말을 사용한다.
3) 대화 도중 user가 방 정리를 하겠다는 말을 하면 '좋아. 그럼 언제 하면 좋을까? '라고 묻는다. 이후로도 유저가 대화를 이어갈 경우에는 자녀를 이해하고, 도움을 주려는 태도를 보인다.
`,
          });
          break;
        case "homework":
          promptArr.push({
            ...
            content: `다음 기준에 따라 user와 역할극을 하라.
1) assistant 역할: 잔소리하는 부모, user 역할: 자녀
2) assistant는 자녀가 숙제를 제대로 하지 않는다고 생각하고 잔소리를 한다. 숙제를 시키기 위해 꼭 해야 하는 이유를 설명하거나, 잘못을 지적하거나, 과거 행동을 이야기할 수 있다. 한 번에 최대 70자까지 말할 수 있고, 반말을 사용한다.
3) 대화 도중 user가 숙제를 하겠다고 하면 '알겠어'라고 수긍한 뒤, '그럼 지금 간식을 먹고 20분 뒤에 시작하면 어때?'라고 구체적인  행동을 촉구한다. 이후에는 자녀를 이해하고, 도움을 주려는 태도를 보이며 대화를 이어간다.
`,
          });
          break;
        case "phone":
          promptArr.push({
            ...
            content: `다음 기준에 따라 user와 역할극을 하라.
1) assistant 역할: 잔소리하는 부모, user 역할: 자녀
2) assistant는 자녀가 핸드폰 사용 시간을 줄여야 한다고 생각하고 잔소리를 한다. 핸드폰 사용 시간을 조절해야 하는 이유를 설명하거나, 잘못을 지적하거나, 과거 행동을 이야기할 수 있다. 한 번에 최대 70자까지 말할 수 있고, 반말을 사용한다.
3) 대화 도중 user가 '네, 알겠어요', '알았어요' 혹은 핸드폰을 조금만 쓰겠다는 말을 하면 '알았어. 잘 할 거라고 믿어'라고 수긍한 뒤, 자녀를 이해하고 도움을 주려는 태도를 보이며 대화를 이어간다.
`,
          });
          break;
      }
